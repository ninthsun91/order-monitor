...
- [ ] **(2026-07-22 백로그)** 추적 벽(관측 하한 100 이상) 수량 궤적 로깅 — 레지스트리 등록 시 + 리필(수량 증가 델타) 시 구조화 로그 1줄씩. 현재는 소멸만 로그라 준임계 벽의 리필 횟수·총량이 무기록 (`events` 테이블의 선행 단계, 근거: 결정 기록 2026-07-22 — 07-17 62.8k 사례)
...
| 2 | 케이스 2 "상위 구간" 범위 제한 필요 여부 | M4 설계 시 | 🟠 보류 | M4는 거리 무제한으로 구현·첫 실전 알림 1건 수신(2026-07-14). 검토 결과 귀속 불확실성이 거리에 비례함이 확인됐으나 표본 1건으로 상한 도입 판단 불가 → D4·케이스 2 자체가 v1.6에서 임시 비활성(2026-07-15 결정 기록) — 재도입 논의 때 거리 상한을 함께 결정 |
