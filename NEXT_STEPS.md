...
**스펙 기준**: PRD **§8 D4 (v1.11)** 전체 — 산식(가시 리필 페어링 + 틱 단위 은닉 대조), 배수 판정, 스트릭/래치/진행/종결 수명, **관측 기록** 문단까지 구현 범위다.
...
- **D2 요약(verdict)에 근접 벽 컨텍스트 첨부** — 종가 부근 추적 벽 peak/last 동봉. 구현 위치: `alerting/dispatcher.py` D2 요약 포맷 + wall_registry 조회
