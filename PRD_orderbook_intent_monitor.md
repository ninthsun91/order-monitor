...
레벨 소멸·케이스 1/2 충족·epoch 종료로만 발생 (구 INTENT_TTL 폐지, 개정 이력 참고)
```
...
**종국 상태 목록 (v1.2, v1.5에서 EXPIRED 폐지)** — 인텐트는 다음 다섯 상태 중 하나로 종결된다:
...
| D5 `PARTIALLY_EXECUTED` / `INTENT_WITHDRAWN` / `INTERRUPTED` | 발송 안 함 (로그/DB만) | 튜닝·사후 분석 데이터 (§8 D5 종국 상태 목록, v1.5에서 EXPIRED 폐지) |
