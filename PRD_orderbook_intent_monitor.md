...
**등록 크기 S (v1.2 명시)**: S는 **D1 APPEARED 발화 시점의 `current_size`로 고정**한다 (지속 필터 통과 직후의 관측값). peak를 쓰지 않는 이유: 순간 스파이크가 실현률 분모를 부풀린다. 등록 후 벽이 성장/축소해도 S는 갱신하지 않는다 — 실현률의 기준은 "등록을 촉발한 표시 의도"이며, 이후 크기 변화는 로그/DB(peak 등)로만 기록한다 (수용 한계).
