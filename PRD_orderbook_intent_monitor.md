...
|---|---|
| 문서 버전 | v1.10 |
| 작성일 | 2026-07-11 (v1.1/v1.2 개정 동일, v1.3은 2026-07-12, v1.4는 2026-07-13, v1.5는 2026-07-14, v1.6/v1.7은 2026-07-15, v1.8/v1.9/v1.10은 2026-07-22) |
| 상태 | Draft |
...
> **(v1.6) 임시 비활성** — 배선에서 제외됨. 본 절의 명세는 재도입 논의의 기준으로 유지한다 (개정 이력 v1.6 참고).
...
- 500ms 근거: 아이스버그 엔진의 리필은 통상 즉각적이나 스냅샷 해상도가 100ms이므로 여유 포함. M6 실운영 로그 리뷰의 튜닝 대상 (v1.4 — Bookmap 대조 기각, §15 #5)
...
**파라미터**: `REALIZE_PCT`(0.6), `REALIZE_PCT_ABOVE`(0.6), 케이스 2의 "상위 구간" 정의 = 의도 레벨과 현재가 사이의 같은 side 레벨들.
...
**종국 상태 목록 (v1.2, v1.5에서 EXPIRED 폐지, v1.9에서 CONFIRMED 래치화·CONFIRMED_CLOSED 신설)**:

| 상태 | 알림 | 의미 |
|---|---|---|
| `EXECUTION_CONFIRMED` | 발송 | 케이스 1 — 레벨 체결 임계 충족. **(v1.9) 종국이 아닌 래치**: 발화 1회 후 인텐트는 유지되어 진행률 감시가 계속된다 |
| `EXECUTION_INFERRED_ABOVE` | 발송 (추정) | 케이스 2 — 상위 구간 추정 누적 충족 **(v1.6 비활성 — D4 미배선으로 도달 불가)** |
...
**소멸 시 종국 평가 (v1.2)**: 등록된 인텐트의 레벨이 소멸하면(D1 REMOVED, tombstone, 기록 하한 미만 하락 — §8 D1 소멸 규칙) TTL이나 다음 이벤트를 기다리지 않고 **즉시** 종국 상태를 평가한다. **(v1.9)** 이미 확정 래치된 인텐트는 아래 분류와 무관하게 `CONFIRMED_CLOSED`(로그만)로 마감한다. 미확정 인텐트의 소멸 원인은 네 가지로 구별한다:
...
- 발화는 `(인텐트, 계열, 경계값)`당 1회. **(v1.9)** 확정 **전**에는 확정/추정 임계(`REALIZE_PCT` 0.6) 도달 경계를 확정/추정 알림 자체가 대체하므로 기본값 기준 실효 경계는 20%, 40%다. 확정 **후**(케이스 1 래치)에는 경계 상한 없이 계속 발화한다 — 80%, 100%, 120%, … (100% 초과 = 리필분 재흡수, 그 구간의 총 흡수량 가시화)
...
| D5 `EXECUTION_INFERRED_ABOVE` | **발송** (추정 등급 표기) | 케이스 2 — 메시지에 "추정" 명시, 귀속 한계는 §8 D5 판정 의미 참조 **(v1.6 비활성)** |
...
| 아이스버그 휴리스틱 오탐/미탐 | 케이스 2 신뢰도 저하 | 실운영 로그 리뷰로 튜닝(M6, v1.4 — Bookmap 대조 기각), 보수적 임계치, D4 단독 알림 금지 |
